...
model <- function(parameters, statistic='distance', parallel=TRUE) {
  ...
  if (parallel) {
    ...
    clusterExport(cl,
      c("one_parameter","simulate_continuous_moran_tau",
...
for (CANCER_TISSUE in CANCER_TISSUES) {
  for (WGD_STATUS in c(FALSE, TRUE)) {
    message("——————————————————————————————")
    message(" Processing tissue: ", CANCER_TISSUE,
            " | WGD = ", WGD_STATUS)
    message("——————————————————————————————")
    ...
    file.rename("comparison-marginal-parameter=lambda.png",
                paste0(CANCER_TISSUE,
                       "_WGD=",WGD_STATUS,"_lambda.png"))

    # 5) sim vs target CDF
    finals   <- smcrf_results[[paste0("Iteration_",
                                     NUM_ITERATIONS+1)]]$parameters
    sims_df  <- model(
    ...
    sim_long <- sims_df %>%
      ...
      mutate(bin=as.integer(sub("Age_bin_","",bin)),
    ...
    p <- ggplot() +
      ...
      geom_line(data=target_df,
                aes(mid,cdf),
                color="firebrick", size=1.2) +
      theme_minimal(base_size=14) +
      labs(title=paste0(CANCER_TISSUE,
                        " (WGD=",WGD_STATUS,")"),
           x="MRCA age", y="Cumulative frequency")

    ggsave(paste0(CANCER_TISSUE,
                  "_WGD=",WGD_STATUS,".jpg"),
           p, dpi=150, width=12, height=6, units="in")
  }
}
