...
#----------------------------------------with samples with purity >= 30%
cancer_type <- "Colorectal Cancer"
clinical_data <- clinical_data %>%
    filter(
        SAMPLE_TYPE %in% c("Primary"),
...
#-------------------------------------Limit to KRAS, APC, TP53 mutations
mutation_data <- mutation_data %>%
    filter(Hugo_Symbol %in% c("KRAS", "APC", "TP53"))
#----------------------------------Limit to mutations with tumor CN <= 2
mutation_data <- mutation_data %>%
    filter(!is.na(normal_cn) & !is.na(tumor_cn) & tumor_cn <= 2)
...
#-------If there are >= 1 mutations in the same gene in the same sample:
#--------------------------retain only the mutation with the highest CCF
mutation_data <- mutation_data %>%
    group_by(Tumor_Sample_Barcode, Hugo_Symbol) %>%
    filter(CCF == max(CCF)) %>%
    ungroup()
#-----------------------------------------Report some VAF/CCF statistics
cat("\nBEFORE FILTERING:--------------------------------------------------\n")
...
write.table(mutation_data, file = paste0(R_workplace, "/Colorectal_drivers.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
