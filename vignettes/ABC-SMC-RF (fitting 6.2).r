...
model <- function(parameters, statistic='distance', parallel=TRUE) {
  one_parameter <- function(parameter) {
    ###### (a) Extract the five “to-be-inferred” parameters
    ...
    # Reconstruct actual r₀ and threshold from their logs
    r0        <- 10^log10_r0
    threshold <- 10^log10_thresh

    # Fixed (non‐inferred) pieces
    u_vec      <- c(0, 0)
    tau        <- 0.01
    n_sim      <- 100
  ...
  ###### (g) Apply one_parameter() to every row of “parameters”
  if (parallel) {
  ...
  ###### (h) Append the returned statistics onto the original “parameters” DF
  cbind(parameters, stats)
}

####### 2b) PRIOR / DENSITY FUNCTIONS
rprior <- function(Nparameters) {
  # 1) log10(r₀) ∼ Uniform(2,6)
  log10_r0_vals     <- runif(Nparameters, 2, 6)
  # 2) λ₁ ∼ Uniform(0,20)
  lambda1_vals      <- runif(Nparameters, 0, 20)
  # 3) λ₀ ∼ Uniform(0, λ₁)
  lambda0_vals      <- mapply(
    function(l1) runif(1, 0, l1),
    lambda1_vals
  )
  # 4) log10(threshold) ∼ Uniform(1, log10(r₀))
  log10_thresh_vals <- mapply(
    function(lr0) runif(1, 1, lr0),
    log10_r0_vals
  )
  # 5) α ∼ Uniform(0.5,1)
  alpha_vals        <- runif(Nparameters, 0.5, 1)

  data.frame(
    log10_r0        = log10_r0_vals,
    lambda1         = lambda1_vals,
    lambda0         = lambda0_vals,
    log10_threshold = log10_thresh_vals,
    alpha           = alpha_vals
  )
}
...
dprior <- function(parameters, parameter_id="all") {
  n <- nrow(parameters)
  # p(log10_r₀) = 1/(6−2) on [2,6]
  p_r0 <- dunif(parameters$log10_r0, 2, 6)
  # p(λ₁) = 1/20 on [0,20]
  p_l1 <- dunif(parameters$lambda1, 0, 20)
  # p(λ₀|λ₁) = 1/λ₁ if 0 ≤ λ₀ ≤ λ₁
  p_l0 <- rep(0, n)
  idx0 <- (parameters$lambda0 >= 0) & (parameters$lambda0 <= parameters$lambda1)
  p_l0[idx0] <- 1 / parameters$lambda1[idx0]

  # p(log10(threshold) | log10_r₀) = 1/(log10_r₀ −1) if 1 ≤ log10_threshold ≤ log10_r₀
  p_t  <- rep(0, n)
  idx_t <- (parameters$log10_threshold >= 1) &
           (parameters$log10_threshold <= parameters$log10_r0)
  p_t[idx_t] <- 1 / (parameters$log10_r0[idx_t] - 1)

  # p(α) = 1/(1 − 0.5) = 2 on [0.5,1]
  p_a <- dunif(parameters$alpha, 0.5, 1)

  joint_p <- p_r0 * p_l1 * p_l0 * p_t * p_a
  return(joint_p)
}
...
for (CANCER_TISSUE in CANCER_TISSUES) {
  for (WGD_STATUS in c(FALSE, TRUE)) {
    ...
    real_sub <- real_df %>%
      ...
      rename(MRCA_age = MRCA.time.linear,
             diag_age  = age) %>%
    ...
    file.rename("comparison-marginal-parameter=lambda1.png",
