...
for (row in 1:nrow(inference_list)) {
    Sample_ID <- inference_list$Sample_ID[row]
    ...
    # ####################################################################
    # ####################################################################
    if (Sample_ID != "PD41165" | Gene_ID != "DNMT3A" | Protein_change_ID != "p.I705T") next
    # ####################################################################
    # ####################################################################
    # ####################################################################
    # ####################################################################
    # ####################################################################
    outdir <- paste0("Results_", Gene_ID, "_", Protein_change_ID, "_", Sample_ID)
    if (!dir.exists(outdir)) dir.create(outdir, recursive = TRUE)
    #---Compute log posterior distribution
    grid <- basic_grid
    timing <- system.time({
        #---Parallel version with annotation
        library(pbapply)
        ...
        loglikelihoods <- pblapply(cl = cl, X = 1:nrow(grid), FUN = function(i) {
    ...
    saveRDS(grid, file = file.path(outdir, paste0("posterior_", Gene_ID, "_", Protein_change_ID, "_", Sample_ID, ".rds")))
    write.csv(grid,
        file = file.path(outdir, paste0("posterior_", Gene_ID, "_", Protein_change_ID, "_", Sample_ID, ".csv")),
        row.names = FALSE
    )
    ...
    pdf(file.path(outdir, paste0("Marginal_distributions_", Gene_ID, "_", Protein_change_ID, "_", Sample_ID, ".pdf")), width = 20, height = 10)
    p <- gridExtra::arrangeGrob(p1, p2,
        ncol = 2,
        top = paste0("Marginal posterior distributions (", Gene_ID, "-", Protein_change_ID, "; ", Sample_ID, ")")
    )
    grid.draw(p)
    dev.off()
    ...
    pdf(file.path(outdir, paste0("Joint_distributions_", Gene_ID, "_", Protein_change_ID, "_", Sample_ID, ".pdf")), width = 10, height = 10)
    p <- gridExtra::arrangeGrob(p,
        ncol = 1,
        top = paste0("Joint posterior distributions (", Gene_ID, "-", Protein_change_ID, "; ", Sample_ID, ")")
    )
    grid.draw(p)
    dev.off()
    #---Plot VAF dynamics
    ...
    pdf(file.path(outdir, paste0("VAF_trajectories_", Gene_ID, "_", Protein_change_ID, "_", Sample_ID, ".pdf")), width = 30, height = 15)
    print(p)
    dev.off()
}
