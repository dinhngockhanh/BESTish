...
#---------Input list of MutationID x Study pairs in Watson for inference
inference_list <- read.csv("watson_inference_list_filtered.csv")
#------------------------------------------------------Input Watson data
watson <- read.csv("watson_master.csv")
watson$Age <- as.numeric(watson$Age)
watson$VAF <- as.numeric(watson$VAF)
for (row in 1:nrow(inference_list)) {
    Study <- inference_list$Study[row]
    Gene_ID <- inference_list$Gene_ID[row]
    Protein_change_ID <- inference_list$Protein_change_ID[row]
    watson_short <- watson[
        watson$Study == Study &
            watson$Gene_ID == Gene_ID &
            watson$Protein_change_ID == Protein_change_ID,
    ]
    watson_short <- watson_short[order(watson_short$Age), ]
    patient_age <- watson_short$Age
    patient_vaf <- watson_short$VAF
    outdir <- paste0("Results_", Gene_ID, "_", Protein_change_ID, "_", Study)
    ...
    saveRDS(grid, file = file.path(outdir, paste0("posterior_", Gene_ID, "_", Protein_change_ID, "_", Study, ".rds")))
    write.csv(grid,
        file = file.path(outdir, paste0("posterior_", Gene_ID, "_", Protein_change_ID, "_", Study, ".csv")),
        row.names = FALSE
    )
    ...
    pdf(file.path(outdir, paste0("Marginal_distributions_", Gene_ID, "_", Protein_change_ID, "_", Study, ".pdf")), width = 30, height = 10)
    p <- gridExtra::arrangeGrob(p1, p2, p3,
        ncol = 3,
        top = paste0("Marginal posterior distributions (", Gene_ID, "-", Protein_change_ID, "; ", Study, ")")
    )
    grid.draw(p)
    dev.off()
    ...
    pdf(file.path(outdir, paste0("Joint_distributions_", Gene_ID, "_", Protein_change_ID, "_", Study, ".pdf")), width = 30, height = 10)
    p <- gridExtra::arrangeGrob(p12, p13, p23,
        ncol = 3,
        top = paste0("Joint posterior distributions (", Gene_ID, "-", Protein_change_ID, "; ", Study, ")")
    )
    grid.draw(p)
    dev.off()
    #---Plot VAF dynamics
    p <- ggplot() +
        ...
        geom_point(data = watson_short, aes(x = Age, y = VAF), color = "white", size = 14) +
        geom_point(data = watson_short, aes(x = Age, y = VAF), color = "#00A087", size = 10) +
        labs(
            x = "Age",
    ...
    pdf(file.path(outdir, paste0("VAF_trajectories_", Gene_ID, "_", Protein_change_ID, "_", Study, ".pdf")), width = 30, height = 15)
    print(p)
    dev.off()
}
